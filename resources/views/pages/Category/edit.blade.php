    ...
    <div class="content" data-select2-id="11">
      ...
      <div class="row ">
        <div class="col-xl-12">
          <div class="card">
            <div class="card-body">
              <h4 class="page-title"> <i class="mdi mdi-apple-keyboard-command title_icon"></i> Update category</h4>
            </div> <!-- end card body-->
          </div> <!-- end card -->
        </div><!-- end col-->
      </div>
          ...
          <div class="card" data-select2-id="8">
            <div class="card-body" data-select2-id="7">
              <div class="col-lg-12" data-select2-id="6">
                ...
                <form class="required-form" action="{{URL::to('Category',$categories->id)}}" method="post" enctype="multipart/form-data" data-select2-id="5">
                    @csrf
                    @method('PUT')
                  <div class="form-group">
                    <label for="code">Category code</label>
                    <input type="text" class="form-control" id="code" name="category_code" value="{{$categories->category_code}}" readonly="">
                  </div>
  
                  <div class="form-group">
                    <label for="name">Category title<span class="required">*</span></label>
                    <input type="text" class="form-control" id="name" name="category_title" value="{{$categories->category_title}}" required="">
                  </div>
  
                  <div class="form-group" data-select2-id="4">
                    <label for="parent">Parent</label>
                    <select class="form-control select2 select2-accessible"  name="category_parent" id="parent" onchange="checkCategoryType(this.value)" onchange="checkCategoryType(this.value)" data-select2-id="parent" tabindex="-1" aria-hidden="true">
                      <option value="0" data-select2-id="2">None</option>
                     
                      @foreach ($category as $category)
<option value="{{$category->id}}"
{{$category->id == $categories->category_parent ? 'selected' : ''}}
>
...
@endforeach
                  ...
                  <div class="form-group iconpicker-container" id="icon-picker-area" style="">
                    ...
                    <input type="text" id="font_awesome_class" name="category_icon" value="{{$categories->category_icon}}" class="form-control icon-picker iconpicker-element iconpicker-input" autocomplete="off">
